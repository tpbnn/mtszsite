...
let schedules = [
  {
    ...
    desc:'S~D 티어 밸런스 팀전',
    comments:[
      {nick:'시청자1', text:'오늘 엔트리 기대됩니다.'}
    ]
  },
  {
    date:'2026-05-24',
    title:'시청자 참여 스타 대회',
    desc:'무작위 신청자와 크루 멤버 이벤트전',
    comments:[
      {nick:'팬', text:'저도 참가하고 싶어요!'}
    ]
  }
];
...
async function loadTierExcel(){
  ...
  const TIER_SHEET_URL =
    "https://docs.google.com/spreadsheets/d/1jjguKLQE4dI76DBIbEw4ppxG26Ld8Eq7_cvbqFYU7Yo/gviz/tq?tqx=out:csv&sheet=tier";


  const response =
    await fetch(TIER_SHEET_URL);
...
function setupSelects(){
  ...
  if(tierFilterMenu){
    ...
    */
    ...
    if(
      typeof window.restoreTierMultiFilters
      === 'function'
    ){

      window.restoreTierMultiFilters();
  ...
  if(
    playerB &&
    playerB.options.length > 1
  ){

    playerB.selectedIndex = 1;
    ...
    members
      .map(
        m => {
          ...
          const stationUrl =
            m.station ||
            `https://ch.sooplive.co.kr/${m.soopId}`;
...
function renderTierCards(){
  ...
  const selectedTiers =
    Array.from(
      document.querySelectorAll(
        'input[name="tierFilterMulti"]:checked'
      )
    )
    .map(
      el =>
  ...
  const selectedRaces =
    Array.from(
      document.querySelectorAll(
        'input[name="raceFilterMulti"]:checked'
      )
    )
    .map(
      el =>
  ...
  return {
    ...
    url:
      ...
      (
        ...
        `https://www.sooplive.com/station/${channel.soopId}/post/${titleNo}`
...
function sortPostsByRecent(list){
...
async function saveMemberToFirebase(){
  ...
  await addDoc(
...

    {
      ...
      station:
        `https://ch.sooplive.co.kr/${soopId}`,
...
async function editMemberFromFirebase(id){
  ...
  await updateDoc(
...

    {
      ...
      station:
        `https://ch.sooplive.co.kr/${soopId.trim()}`
