...
function renderPosts(){
  ...
  postListEl.innerHTML = visiblePosts.map(p => `<a class="post-item" href="${escapeHtml(p.url || '#')}" target="_blank"><div class="post-head"><img class="post-avatar" src="${escapeHtml(p.profileImage || '')}" onerror="this.style.display='none'"><div class="post-title-area"><div class="name">${escapeHtml(p.title || '제목 없음')}</div><div class="meta">${escapeHtml(p.name || p.author || '')} · ${escapeHtml(p.date || p.createdAt || '')}</div></div></div><div class="post-preview">${escapeHtml(p.content || p.description || '')}</div></a>`).join('') || `<p class="meta">게시글을 불러올 수 없습니다...<br>잠시 기다려 주세요...</p>`;
